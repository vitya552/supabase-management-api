...
describe('withTransaction', () => {
  it('commits related writes on one connection', async () => {
    ...
    await withTransaction(async (tx) => {
      ...
      await upsertConfig('default', { MAILER_TEMPLATES_INVITE_CONTENT: '<p/>', SMTP_PASS: 'x' }, tx)
    })
  ...
  it('rolls back everything when a later statement fails', async () => {
    ...
    await assert.rejects(
      withTransaction(async (tx) => {
        await deleteEmailTemplate('default', 'invite', tx)
        await deleteConfig('default', ['MAILER_TEMPLATES_INVITE_CONTENT'], tx)
      }),
      /boom/
    )
  ...
  it('does not nest transactions when a client is passed to upsertConfig', async () => {
    ...
    await upsertConfig('default', { SITE_URL: 'http://a', DISABLE_SIGNUP: null }, client)
...
describe('sensitive config serialization', () => {
  it('encrypts SMS_TEST_OTP before it reaches the database', async () => {
    ...
    await upsertConfig('default', { SMS_TEST_OTP: mapping, SITE_URL: 'http://a' }, client)

    const otpInsert = statements.find((s) => s.values?.[1] === 'SMS_TEST_OTP')
    assert.ok(otpInsert)
    const stored = String(otpInsert.values?.[2])
    assert.ok(!stored.includes('123456'))
    assert.ok(!stored.includes('+15555550100'))
    assert.match(stored, /^"enc:v2:/)

    const siteInsert = statements.find((s) => s.values?.[1] === 'SITE_URL')
    assert.equal(siteInsert?.values?.[2], JSON.stringify('http://a'))
  })
})
