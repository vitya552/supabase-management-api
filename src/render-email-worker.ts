/**
 * Renders a react-email template in a throwaway child process.
 *
 * Template sources are user supplied, so they are never evaluated inside the
 * API process: this worker is spawned with an empty environment (no database
 * URL, no encryption key, no service credentials) and only exposes a fixed
 * allowlist of modules to the template, so a malicious template cannot read
 * managed secrets or reach the rest of the stack.
 *
 * Protocol: TSX source on stdin, `{ html }` or `{ error }` JSON on stdout.
 */
...
/** Modules a template may import. Anything else is rejected. */
