/**
 * Default react-email sources for every GoTrue mailer template. Bodies mirror
 * the stock Supabase Auth templates so out-of-the-box emails look identical,
 * while giving users an editable react-email starting point.
 */

interface TemplateDefault {
  subject: string
  heading: string
  /** JSX lines placed after the heading inside the container. */
  body: string
}
