...
function decodeBase64(value: string, expectedBytes?: number): Buffer {
  ...
  // Buffer.from ignores invalid characters, so re-encoding is what actually
  // rejects a tampered value.
...
/**
 * Encrypts a value with AES-256-GCM. `context` (the config key or secret
 * name) is authenticated as additional data, so a ciphertext cannot be moved
 * from one key to another by anyone with write access to the database.
 */
...
export function decryptString(value: string, context = ''): string {
  ...
  // v1 ciphertexts predate context binding.
